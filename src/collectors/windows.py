...
class WindowsCollector(BaseCollector):
    ...
    def _process_event(self, event, channel):
        ...
        raw_data = {
            ...
            "level": event.EventType, # Using standard 'level' field for windows EventType (Warning/Error/Info)
            "severity": event.EventType, # Keeping legacy field just in case
            "event_id": event.EventID & 0xFFFF,
            "message": msg,
            "raw_log": msg, # Windows logs don't have a single "raw line", message is closest
            "original_record_number": event.RecordNumber
        }
        
        # Merge extra data before processing
        if extra_data:
            raw_data.update(extra_data)

        # Use centralized processor
        log_entry = self.processor.process(raw_data)
        
        # Restore specific windows metadata if processor obscured it (processor puts raw_data in metadata, but we want ID at top level for dashboards if needed)
        log_entry['event_id'] = event.EventID & 0xFFFF
