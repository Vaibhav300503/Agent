...
class SimpleRuleEngine:
    ...
    def _match_rule(self, rule, parsed, raw_log):
        ...
        if "event_code" in conditions:
            if parsed.get("event_code") != conditions["event_code"]:
                return False
    ...
    def _check_threshold(self, rule, parsed, current_log):
        ...
        query = {
            ...
            "parsed_data.event_code": conditions.get("event_code")
        }
        
        # Add filters for grouping
        if parsed.get("source_ip"):
            query["parsed_data.source_ip"] = parsed["source_ip"]
        if parsed.get("user_name"):
            query["parsed_data.user_name"] = parsed["user_name"]
    ...
    def _create_alert(self, rule, raw_log, parsed):
        ...
        alert = {
            ...
            "agent_hostname": raw_log.get("hostname"),
