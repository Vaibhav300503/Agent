...
async def ingest_logs(logs: List[LogEntry], token: str = Depends(verify_token)):
    """
    ...
    try:
        ...
        for log in logs:
            doc = {
                "agent_id": log.hostname,  # Use hostname as agent_id for now
                "hostname": log.hostname,
                "ip_address": log.ip_address,
                "timestamp": datetime.fromisoformat(log.timestamp.replace('Z', '+00:00')),
                "raw_data": log.dict(),
                "processed": False,
                "has_alert": False,  # Smart retention flag
